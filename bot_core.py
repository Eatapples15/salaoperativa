...
# Configura il logging per l'intero script del bot.
# Questo ti aiuterà a vedere cosa sta succedendo nei log di Render.com.
...
# Questa variabile traccia l'ultima data del bollettino che è stata processata.
# NON è persistente tra i riavvii del servizio su Render Free Tier.
# Per una persistenza reale, dovresti salvare questo valore su un file JSON o un piccolo DB.
last_processed_bollettino_date = None # Dovrebbe essere un oggetto datetime.date

# --- Istanza dell'Applicazione Telegram ---
# L'istanza dell'applicazione `python-telegram-bot`
application = None
